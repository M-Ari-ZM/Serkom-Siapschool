<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-xl font-bold text-gray-800">Tambah FAQ Baru</h1>
                <p class="text-sm text-gray-500 mt-0.5">Isi pertanyaan dan jawaban untuk ditampilkan di landing page.</p>
            </div>
            ...
            <div class="bg-white rounded-xl border border-gray-100 p-6 sm:p-8">
                <form action="{{ route('admin.faqs.store') }}" method="POST" class="space-y-5">
                    ...
                    <div>
                        ...
                        <input id="question" name="question" value="{{ old('question') }}"
                            placeholder="Contoh: Berapa lama proses implementasi?"
                            required
                    ...
                    <div>
                        ...
                        <textarea id="answer" name="answer" rows="4"
                            placeholder="Jelaskan jawaban secara informatif..."
                            required
                            class="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:border-transparent transition-all resize-none">{{ old('answer') }}</textarea>
                        @error('answer')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
