<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-xl font-bold text-gray-800">Tambah Fitur Baru</h1>
                <p class="text-sm text-gray-500 mt-0.5">Isi data fitur yang akan ditampilkan di landing page.</p>
            </div>
    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-xl border border-gray-100 p-6 sm:p-8">
                <form action="{{ route('admin.features.store') }}" method="POST" class="space-y-5">
                    @csrf

                    <div>
                        <label for="title" class="block text-sm font-medium text-gray-700 mb-1.5">Judul Fitur</label>
                        <input id="title" name="title" value="{{ old('title') }}"
                            placeholder="Contoh: Presensi QR Code"
                            required
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="4"
                            placeholder="Jelaskan manfaat utama fitur ini bagi sekolah..."
                            required
                            class="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:border-transparent transition-all resize-none">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    {{-- Icon Picker --}}
                    <div class="pt-1">
                        <x-admin.icon-picker :selected="old('icon')" />
                        @error('icon')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
