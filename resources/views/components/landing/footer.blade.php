                ...
                <ul class="space-y-2.5 text-sm">
                    ...
                    @else
                        <li class="flex items-center gap-2">
                            ...
                            <span>info@siapschool.com</span>
                        </li>
                    @endif

                    @if($setting->whatsapp_cs ?? false)
                        @php
                            $cleanWa = preg_replace('/[^0-9]/', '', $setting->whatsapp_cs);
                            if (str_starts_with($cleanWa, '0')) {
                                $cleanWa = '62' . substr($cleanWa, 1);
                            }
                        @endphp
                        <li class="flex items-center gap-2">
                            <svg class="w-4 h-4 text-teal-400 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
                            ...
                            <a href="https://wa.me/{{ $cleanWa }}" target="_blank" rel="noopener noreferrer" class="hover:text-white transition-colors">{{ $setting->whatsapp_cs }}</a>
                        </li>
                    @endif
                </ul>
            </div>
